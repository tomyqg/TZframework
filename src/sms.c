...
uint8 SmsLock(uint8 p_in[],uint8 p_out[])
{
	...
	sys_work_para_struct.sms_lock_car_flag = VALID_VAL_AA;
	bias_addr = ((uint8*)&sys_work_para_struct.sms_lock_car_flag) - ((uint8*)&sys_work_para_struct.acc_on_sec_statistic_counter);
	res = SpiFramWrite(SYS_WORK_PARA_STRUCT_START_ADDR+bias_addr,(uint8*)&sys_work_para_struct.sms_lock_car_flag,1);
	if(res)
	{
		ack_flag = 'Y';
	}
...
uint8 SmsUnlock(uint8 p_in[],uint8 p_out[])
{
	...
	sys_work_para_struct.sms_lock_car_flag = VALID_VAL_BB;
	bias_addr = ((uint8*)&sys_work_para_struct.sms_lock_car_flag) - ((uint8*)&sys_work_para_struct.acc_on_sec_statistic_counter);
	res = SpiFramWrite(SYS_WORK_PARA_STRUCT_START_ADDR+bias_addr,(uint8*)&sys_work_para_struct.sms_lock_car_flag,1);
	if(res)
	{
